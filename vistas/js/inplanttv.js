...
function obtenerValor(el) {
  ...
  $.ajax({
    ...
    success: function (data) {
      $("#tablaDatosmaq").html(data.responseText);
      ocultarLoader();
    ...
    error: function (response) {
      $("#tablaDatosmaq").html(response.responseText);
...
function obtenerFecha(el) {
  
  document.getElementById('txtFecha').value="";
  let index = $(el).closest("tr").index();
  let table = document.getElementById("tablaDatosmaq");
  let id = parseInt(table.rows[index + 1].cells[0].innerText);
  let nombre=table.rows[index + 1].cells[1].innerText;
  let fcambio = table.rows[index+1].cells[2].innerText;
  let pod= table.rows[index + 1].cells[3].innerText;
  let tipo = table.rows[index + 1].cells[6].innerText;
  let tipomanto = table.rows[index + 1].cells[7].innerText;
  let procesador= table.rows[index + 1].cells[8].innerText;
  let operacion="";


  debugger;

  if (tipo == 'MANGUERA'  && tipomanto=='cambio') {
    operacion="vuelta"
  }else if(tipo == 'MANGUERA'  && tipomanto=='vuelta'){
    operacion="cambio"
  }else if(tipo == 'MANGUERA'  && tipomanto==""){
    operacion="cambio";
  }else if(tipo == 'BOQUILLA'){
    operacion="cambio";
  }else if(tipo == 'POT'){
    operacion="cambio";
  }else{
  ...
  document.getElementById("txtFcambio").value=fcambio;
  document.getElementById("txtPod").value=pod;
  document.getElementById("txtTipo").value=tipo;
  document.getElementById("txtTipomanto").value=tipomanto;
  document.getElementById("txtProcesador").value=procesador;
  document.getElementById("txtOperacion").value=operacion;

  $(".modal-header").css("background-color", "#07B5E8");
  $(".modal-title").text("Modificación De Fecha");
  $("#ModalRep").modal("show");
}
...
function miFunc(){
  ...
  if(document.getElementById('txtFecha').value==""){
    Swal.fire({
      position: "center",
      icon: "error",
      title: "Favor de selecionar una fecha",
      showConfirmButton: false,
      timer: 1500,
    });
    return
  }else{
    let valor=validarFechaMenorActual(document.getElementById('txtFecha').value);
    if (valor==false){
      Swal.fire({
        position: "center",
        icon: "error",
        title: "Favor de ingresar una fecha valida",
        showConfirmButton: false,
        timer: 1500,
      });
      return
  ...
  const pot=document.getElementById("txtPod").value;
  const procesador_pza=document.getElementById("txtProcesador").value;
  const fecha_ini=document.getElementById("txtFcambio").value;
  const fecha_fin=document.getElementById('txtFecha').value;
  const tipo=document.getElementById("txtTipo").value;
  const tipo_mantenimiento=document.getElementById("txtTipomanto").value;
  const id_parte=document.getElementById("txtId").value;
  const operacion=document.getElementById("txtOperacion").value;
  const usuario=document.getElementById("pkuser").value;
  $.ajax({
    url: "modelos/inserthistorico.php",
    type: "POST",
    data: { 
      nombre: nombre,
      pot: pot,
      procesador_pza:procesador_pza,
      fecha_ini:fecha_ini,
      fecha_fin:fecha_fin,
      tipo:tipo,
...
function validarFechaMenorActual(date){
  ...
  var separar =date.split("T");
  var fecha = separar[0].split("-");
  var hora = separar[1];
  // fecha = fecha.split("-");
  fecha = fecha[0] + "-" + fecha[1] + "-" + fecha[2];
  let date1 = moment(fechaHoy).format('YYYY-MM-DD hh:mm');
  let date2 = moment(fecha + " " + hora).format('YYYY-MM-DD hh:mm');
  if (date2 > date1) {
    return false;
  }else{
    return true;
