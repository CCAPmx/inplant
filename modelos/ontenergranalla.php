...
if (isset($_POST["nombre"])) {
    ...
    for ($i = 0; $i < $Contarord; $i++) {
        $consumo_granalla_hora = $requestmaq->response->data[$i]->fieldData->{'consumo_granalla_hora'};
        $granalla_zona_muerta = $requestmaq->response->data[$i]->fieldData->{'granalla_zona_muerta'};
        $granalla_pots = $requestmaq->response->data[$i]->fieldData->{'granalla_pots'};
        $altura_silo = $requestmaq->response->data[$i]->fieldData->{'altura_silo'};
        $no_turbinas_activas = $requestmaq->response->data[$i]->fieldData->{'no_turbinas_activas'};
        $descripcion = $requestmaq->response->data[$i]->fieldData->{'descripcion'};
        $cliente = $requestmaq->response->data[$i]->fieldData->{'cliente'};
        
        $maquinas[] = array('consumo_granalla_hora'=> $consumo_granalla_hora, 'altura_silo'=> $altura_silo, 'granalla_zona_muerta'=> $granalla_zona_muerta, 'granalla_pots'=> $granalla_pots, 'no_turbinas_activas'=> $no_turbinas_activas, 'descripcion'=> $descripcion, 'cliente'=> $cliente);
