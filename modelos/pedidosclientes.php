...
if (isset($_POST["pk"])) {
    ...
    $Contarord = $requestOrden->response->dataInfo->foundCount;
    ...
    for ($i = 0; $i < $Contarord; $i++) {
        ...
        $Orden = $requestOrden->response->data[$i]->fieldData->{'Orden'};
        ...
        $pedidos[] = array('pk'=> $pk, 'Orden'=> $Orden, 'Moneda'=> $Moneda,  'Total'=> $Total, 'Fecha'=> $mifecha );
...
function get_tokendos($host, $username, $password, $payloadName)
{
    $additionalHeaders = '';
    $ch = curl_init($host);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json', $additionalHeaders));
...
function get_dataAlldos($host, $token, $payloadName)
{
    $additionalHeaders = "Authorization: Bearer " . $token;
    $ch = curl_init();
    curl_setopt($ch,  CURLOPT_URL, $host);
...
function get_dataOnedos($host, $token, $payloadName)
{
    $additionalHeaders = "Authorization: Bearer " . $token;
    $ch = curl_init();
    curl_setopt($ch,  CURLOPT_URL, $host);
