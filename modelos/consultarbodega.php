...
if (isset($_POST["fecha"])) {
    ...
    $host = 'https://fms.lersan.com/fmi/data/v1/databases/CCAP/layouts/movimientos/_find?';
    $payloadName = '{"query": [{"fecha": "'. $_POST["fecha"] .'","fkgranalla": "'. $_POST["fkgranalla"] .'"}]}';
    ...
    $tablahoy = array();
         for ($i = 0; $i < $Contar; $i++) {
            try {
                ...
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $contar = 1;
                $descripcion = $requestAll->response->data[$i]->fieldData->{'Maquinas_rel::descripcion'};
                ...
                $sql = "INSERT INTO tembodega (contar, descripcion, tipo, cantidad, fecha) VALUES (:contar, :descripcion, :tipo, :cantidad, :fecha)";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(':contar', $contar);
       ...
       try {
         $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
         $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         $sql = "SELECT id, tipo, CONCAT(sum(cantidad),' ','KG.')  as kilos, descripcion FROM tembodega GROUP BY tipo, descripcion";
         $stmt = $conn->query($sql);
